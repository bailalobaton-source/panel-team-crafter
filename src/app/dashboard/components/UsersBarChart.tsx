...
import { AnalyticsResponse } from "@/src/interfaces/user.type";
import { getUserAnalytics } from "@/src/service/analytics.service";
...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  const { usuarios, index, totalUsuarios } = payload[0].payload;
...
export default function UsersBarChart() {
  const [dataAnalytics, setDataAnalytics] = useState<AnalyticsResponse | null>(
    null,
  );
  ...
  const chartData = useMemo(() => {
    return MONTHS.map((mes, index) => {
      ...
      return {
        mes,
        usuarios: found ? found.total : 0,
        index,
        totalUsuarios,
      };
    });
  }, [dataAnalytics, totalUsuarios]);

  return (
    <div className="w-full h-full bg-white/70 backdrop-blur-xl rounded-2xl p-4 shadow-lg flex gap-4">
      {/* =========================
          LISTA DE MESES
      ========================= */}
      <section className="flex flex-col">
        <h3 className="text-pink-600 text-nowrap text-sm font-semibold mb-3">
          Usuarios registrados en {dataAnalytics?.year}
        </h3>

        <ul className="w-24 lex flex-col gap-1">
          {chartData.map((item) => (
            <li
              key={item.mes}
      ...
      <div className="w-full ">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData}>
            ...
            <Bar dataKey="usuarios" barSize={18} radius={[4, 4, 0, 0]}>
