...
import { postNotificaciones } from "@/src/service/notificaciones.service";
...
export default function ModalNuevaNotificacion({
  open,
  onClose,
  gfindNotificaciones,
}: Props) {
  ...
  const onSubmit = async (data: Notificacion) => {
    try {
      ...
      await postNotificaciones(data); // <-- ahora se envía como multipart/form-data
  ...
  return (
    <Modal isOpen={open} onOpenChange={onClose}>
      ...
      <ModalContent>
        {(onClose) => (
          <>
            <ModalHeader className="pb-0 ">Nueva Notifación</ModalHeader>
            <ModalBody>
              <form
